...
- `minMsi` / `minCoveredMsi` are **observational (0)** for now: the run reports the score without failing the build. Baseline at adoption was **MSI 83.41 %** with **100 % mutation code coverage** (446 mutants, 367 killed, 74 escaped). Raise the floor deliberately once the score has been watched over a few runs — and never by lowering it to accommodate a regression.
