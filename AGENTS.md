...
- `minMsi` / `minCoveredMsi` **gate at 80**. Baseline at adoption was **MSI 83.41 %** with **100 % mutation code coverage** (446 mutants, 367 killed, 74 escaped); the floor sits just below it because a few mutants time out and timeouts are not deterministic, so a floor at the exact baseline would red the build on noise. Raise it as the score improves — and never lower it to admit a regression (fix the tests instead; escaped mutants are tracked in their own issue).
