## Overview
This repository hosts `magicsunday/webtrees-module-base` — a shared PHP library consumed by `webtrees-fan-chart`, `webtrees-pedigree-chart`, `webtrees-descendants-chart`, and `webtrees-statistics`. It contains the common processors (Date, Name, Image, Place), models (Symbols, NameAbbreviation, PlaceStyle, PlaceFormatSpec, PlaceFormatChoice), locale support (`IsoCountryMap`), and module helpers (VersionInformation) those modules use. The three chart modules render genealogy charts; the statistics module renders a statistics dashboard and consumes only the shared core plus `IsoCountryMap`. No JavaScript, no asset pipeline — pure PHP.

Not all of this is consumed by every consumer today. The place subsystem (`PlaceProcessor`, the `PlaceStyle` / `PlaceFormatSpec` / `PlaceFormatChoice` models) and the compact, generation-aware date API (`DateProcessor`'s `getFormatted*` / `get*Full` methods, `CompactDateFormat`, `Symbols`) are currently used only by the fan chart; the pedigree and descendants charts consume just the shared core (name/image processing and `DateProcessor`'s legacy locale-aware `getBirthDate` / `getDeathDate` / `getLifetimeDescription`). `IsoCountryMap` is consumed by the fan chart and the statistics module. These still-fan-only components are kept in the base as deliberate pre-investment for a future second consumer, not because every consumer uses them today — as the statistics module already demonstrated by adopting `IsoCountryMap`.
...
- **`Model/PlaceStyle`** — enum for the way a place name is shortened (`Full`, `Levels`, `CityCountry`, `CityIso2`, `CityIso3` — the last two render the resolved country as an ISO-3166 alpha-2/alpha-3 code instead of its name). Distinct from `PlaceFormatChoice`: "Automatic" is a settings source, resolved before it reaches a formatter.
...
- `minMsi` / `minCoveredMsi` **gate at 82**, measured with `timeoutsAsEscaped: true`. Infection counts a timed-out mutant as *killed* by default, which inflates the score — a mutation that made the suite hang is not one the tests caught; counted honestly, the score at adoption (#39) was **MSI 82.29 %** and the floor was set to 80. Removing the reflection-based tests (#40) and the fetch hardening (#42, #43) raised it to **MSI 84.44 %** at **100 % mutation code coverage** (514 mutants, 434 killed, 75 escaped, 5 timed out), so the floor was ratcheted to 82. The floor sits below the measured score because the timeout count varies with machine load and now feeds the score. There is deliberately no `maxTimeouts` hard limit on top: it would gate on that same non-deterministic quantity a second time without the surrounding score to absorb it. Raise the floor as the score improves — and never lower it to admit a regression (fix the tests instead; escaped mutants are tracked in their own issue).
